...
'$Logfile: /AAMS/Components/bizTravelAgency/bzProductGroup.vb $
'$Workfile: bzProductGroup.vb $
'$Revision: 10 $
'$Archive: /AAMS/Components/bizTravelAgency/bzProductGroup.vb $
'$Modtime: 18/07/08 10:42a $
Imports System.Xml
Imports System.Data.SqlClient
Imports AAMS.bizShared
Namespace bizTravelAgency
    Public Class bzProductGroup
        Implements bizInterface.BizLayerI
        Const strClass_NAME = "bzProductGroup"
        Const gstrList_OUTPUT = "<MS_LISTPRODUCTGROUP_OUTPUT><ProductGroup ProductGroupName='' ProductGroupID='' /><Errors Status=''><Error Code='' Description='' /></Errors></MS_LISTPRODUCTGROUP_OUTPUT>"
        Const srtVIEW_OUTPUT = "<MS_VIEWPRODUCTGROUP_OUTPUT><ProductGroup ProductGroupName='' ProductGroupId=''/><Errors Status=''><Error Code='' Description='' /></Errors></MS_VIEWPRODUCTGROUP_OUTPUT>"
        Const strSEARCH_OUTPUT = "<MS_SEARCHPRODUCTGROUP_OUTPUT><ProductGroup ProductGroupName='' ProductGroupId=''/><PAGE PAGE_COUNT='' TOTAL_ROWS=''/><Errors Status=''><Error Code='' Description='' /></Errors></MS_SEARCHPRODUCTGROUP_OUTPUT>"

        Const StrADD_OUTPUT = "<MS_ADDPRODUCTGROUP_OUTPUT><ProductGroup ACTION='' ProductGroupName='' ProductGroupId=''/></MS_ADDPRODUCTGROUP_OUTPUT>"
        Const strDELETE_OUTPUT = "<MS_DELETEPRODUCTGROUP_OUTPUT><Errors Status=''><Error Code=''  Description='' /></Errors></MS_DELETEPRODUCTGROUP_OUTPUT>"
        Const strUPDATE_OUTPUT = "<MS_UPDATEPRODUCTGROUP_OUTPUT><ProductGroup ACTION='' ProductGroupName='' ProductGroupId=''/><Errors Status=''><Error Code='' Description='' /></Errors></MS_UPDATEPRODUCTGROUP_OUTPUT>"
        ...
        Public Function Delete(ByVal DeleteDoc As System.Xml.XmlDocument) As System.Xml.XmlDocument Implements bizInterface.BizLayerI.Delete
            ...
            Try
                ...
                strPRODUCT_id = DeleteDoc.DocumentElement.SelectSingleNode("ProductGroupId").InnerText.Trim
                ...
                With objSqlCommand
                    ...
                    .CommandText = "UP_SRO_TA_PRODUCT_GROUP"
                    ...
                    .Parameters("@ProductGroupId").Value = strPRODUCT_id
                ...
                Else
                    ...
                    Call bzShared.FillErrorStatus(objDeleteDocOutput, "101", "Product group in use!")
                    Return (objDeleteDocOutput)
                End If
        ...
        Public Function Search(ByVal SearchDoc As System.Xml.XmlDocument) As System.Xml.XmlDocument Implements bizInterface.BizLayerI.Search
            ...
            'Output :
            '<MS_SEARCHPRODUCTGROUP_OUTPUT><ProductGroup ProductGroupName='' ProductGroupId=''/><Errors Status=''><Error Code='' Description='' /></Errors></MS_SEARCHPRODUCTGROUP_OUTPUT>
            '        '************************************************************************
            Dim objSqlConnection As New SqlConnection(bzShared.GetConnectionString())
            Dim objSqlCommand As New SqlCommand
            ...
            Try
                ...
                strST_NAME = (SearchDoc.DocumentElement.SelectSingleNode("ProductGroupName").InnerText.Trim())
                'Paging Section    
                ...
                With objSqlCommand
                    .CommandType = CommandType.StoredProcedure
                    .CommandText = "UP_SER_TA_PRODUCT_GROUP"
                    .Connection = objSqlConnection
                    .Parameters.Add("@ProductGroupName", SqlDbType.VarChar, 100)
                    If strST_NAME = "" Then
                        .Parameters("@ProductGroupName").Value = DBNull.Value
                    Else
                        .Parameters("@ProductGroupName").Value = strST_NAME
                    End If
                ...
                objAptNode = objOutputXml.DocumentElement.SelectSingleNode("ProductGroup")
                ...
                Do While objSqlReader.Read()
                    ...
                    objAptNodeClone.Attributes("ProductGroupId").InnerText = Trim(objSqlReader.GetValue(objSqlReader.GetOrdinal("ProductGroupId")))
        ...
        Public Function Update(ByVal UpdateDoc As System.Xml.XmlDocument) As System.Xml.XmlDocument Implements bizInterface.BizLayerI.Update
            ...
            '</MS_SEARCHORDERSTATUS_OUTPUT>
            'Output :
            '<MS_UPDATEPRODUCTGROUP_OUTPUT><ProductGroup ACTION='' ProductGroupName='' ProductGroupId=''/><Errors Status=''><Error Code='' Description='' /></Errors></MS_UPDATEPRODUCTGROUP_OUTPUT>
            '************************************************************************
            Dim intRetId As Integer
            ...
            Try
                ...
                With objUpdateDocOutput.DocumentElement.SelectSingleNode("ProductGroup")
                    .Attributes("ACTION").InnerText = UpdateDoc.DocumentElement.SelectSingleNode("ProductGroup").Attributes("ACTION").InnerText
                    .Attributes("ProductGroupName").InnerText = UpdateDoc.DocumentElement.SelectSingleNode("ProductGroup").Attributes("ProductGroupName").InnerText
                    .Attributes("ProductGroupId").InnerText = UpdateDoc.DocumentElement.SelectSingleNode("ProductGroup").Attributes("ProductGroupId").InnerText
                End With

                'Retrieving & Checking Details from Input XMLDocument
                With UpdateDoc.DocumentElement.SelectSingleNode("ProductGroup")
                    strAction = ((.Attributes("ACTION").InnerText).Trim).ToString
                    strST_NAME = ((.Attributes("ProductGroupName").InnerText).Trim).ToString

                    If strAction = "I" Or strAction = "U" Then
                        If strST_NAME = "" Then
                            Throw (New AAMSException("Product Group can't be blank."))
                        End If
                    Else
                ...
                With objSqlCommand
                    ...
                    .Parameters.Add(New SqlParameter("@ProductGroupId", SqlDbType.Int))
                    If UpdateDoc.DocumentElement.SelectSingleNode("ProductGroup").Attributes("ProductGroupId").InnerText.Trim <> "" Then
                        .Parameters("@ProductGroupId").Value = UpdateDoc.DocumentElement.SelectSingleNode("ProductGroup").Attributes("ProductGroupId").InnerText
                    Else
                        .Parameters("@ProductGroupId").Value = DBNull.Value
                    End If


                    .Parameters.Add(New SqlParameter("@ProductGroupName", SqlDbType.VarChar, 100))
                    .Parameters("@ProductGroupName").Value = strST_NAME
                    ...
                    If UCase(strAction) = "I" Then
                        intRetId = .Parameters("@RETURNID").Value
                        If intRetId = -1 Then
                            Throw (New AAMSException("Product Group Already Exists!"))
                        Else
                            objUpdateDocOutput.DocumentElement.SelectSingleNode("Errors").Attributes("Status").InnerText = "FALSE"
                            With objUpdateDocOutput.DocumentElement.SelectSingleNode("ProductGroup")
                                .Attributes("ProductGroupId").InnerText = intRetId
                            End With
                        End If
        ...
        Public Function View(ByVal IndexDoc As System.Xml.XmlDocument) As System.Xml.XmlDocument Implements bizInterface.BizLayerI.View
            ...
            'Output :
            '<MS_VIEWPRODUCTGROUP_OUTPUT><ProductGroup ProductGroupName='' ProductGroupId=''/><Errors Status=''><Error Code='' Description='' /></Errors></MS_VIEWPRODUCTGROUP_OUTPUT>
            '************************************************************************
            Dim objSqlConnection As New SqlConnection(bzShared.GetConnectionString())
            Dim objSqlCommand As New SqlCommand
            Dim objOutputXml As New XmlDocument
            ...
            Try
                ...
                '--------Retrieving & Checking Details from Input XMLDocument ------Start
                strST_ID = IndexDoc.DocumentElement.SelectSingleNode("ProductGroupId").InnerText.Trim
                If strST_ID = "" Then
                    Throw (New AAMSException("Incomplete Parameters"))
                End If
                ...
                With objSqlCommand
                    ...
                    .Parameters("@ProductGroupId").Value = strST_ID
                ...
                objSqlReader = objSqlCommand.ExecuteReader()
                Do While objSqlReader.Read()
                    With objOutputXml.DocumentElement.SelectSingleNode("ProductGroup")
                        .Attributes("ProductGroupName").InnerText = Trim(objSqlReader.GetValue(objSqlReader.GetOrdinal("ProductGroupName")))
        ...
        Public Function List() As System.Xml.XmlDocument
            ...
            'Input  : 
            'Output :  
            '<MS_LISTPRODUCTGROUP_OUTPUT>
            '	<ProductGroup ProductGroupName='' ProductGroupID='' />
            '	<Errors Status=''>
            '		<Error Code='' Description='' />
            '	</Errors>
            '</MS_LISTPRODUCTGROUP_OUTPUT>
            ...
            Try
                With objSqlCommand
                    .CommandType = CommandType.StoredProcedure
                    .CommandText = "UP_LST_TA_PRODUCT_GROUP"
                    .Connection = objSqlConnection
                End With
