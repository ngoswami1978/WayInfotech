...
Partial Class ETHelpDesk_HDUP_CallLog
    ...
    Sub checkSecurity()
        ...
        If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
            ...
            If strBuilderFunctional(1) = "0" And strBuilderFunctional(2) <> "0" And (hdPageHD_RE_ID.Value <> "" Or Request.QueryString("HD_RE_ID") IsNot Nothing) Then
                btnSave.Enabled = True
            End If
            If objeAAMS.ReturnViewPermission(Session("Security"), "ET Feedback Action") = "0" Then
                btnFeedBack.Enabled = False
            End If
        Else
    ...
    Sub LoadAllControl()
        Try
            ...
            objeAAMS.BindDropDown(ddlQuerySubGroup, "ET_QuerySubGroup", True, 1)
            objeAAMS.BindDropDown(ddlQueryStatus, "ET_QueryStatusWithClosed", True, 1)
            objeAAMS.BindDropDown(ddlPriority, "ET_QUERYPRIORITY", True, 1)
            objeAAMS.BindDropDown(ddlTeamAssignedTo, "ET_TEAM", True, 1)
            objeAAMS.BindDropDown(ddlContactType, "ET_CONTACTTYPE", True, 1)
            objeAAMS.BindDropDown(ddlMode, "ET_CONTACTTYPE", True, 2)
            objeAAMS.BindDropDown(ddlDisposition, "ET_DISPOSITION", True, 2)
    ...
    Protected Sub theTabStrip_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles theTabStrip.ItemDataBound
        ...
        If Session("Security") IsNot Nothing Then
            ...
            strLogHDCallPermission = objeAAMS.ReturnViewPermission(Session("Security"), "ET Log HD Call")
            Select Case strButtonText
                ...
                Case "Follow Up"
                    ...
                    End If
                Case "Linked LTR"
                    If objeAAMS.ReturnViewPermission(Session("Security"), "ET Linked HD Call") = "0" Then
                        Button1.CssClass = "displayNone"
                    End If
    ...
    Sub fillDropDownList()
        ...
        'Dim objPTR As New AAMS.bizETrackerHelpDesk.bzPTR
        Dim objPTR As New AAMS.bizETrackerHelpDesk.bzPTR
        'Dim objWorkOrder As New AAMS.bizETrackerHelpDesk.bzWorkOrder
        Dim objWorkOrder As New AAMS.bizETrackerHelpDesk.bzWorkOrder
        'Dim objRequestBDR As New AAMS.bizETrackerHelpDesk.bzRequestBDR
        Dim objRequestBDR As New AAMS.bizETrackerHelpDesk.bzRequestBDR
        'Dim objIR As New AAMS.bizETrackerHelpDesk.bzIR
        Dim objIR As New AAMS.bizETrackerHelpDesk.bzIR
...
byPassCondition:
    ...
    Private Sub ViewCallLogInsertDetails()
        ...
        If Session("Security") IsNot Nothing Then
            ...
            If objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default") IsNot Nothing Then
                If objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default/HD_Default").Attributes("FUN_ASSIGNEDTO").Value <> "" Then
                    HdDefaultTeamAssignedTo.Value = objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default/HD_Default").Attributes("FUN_ASSIGNEDTO").Value
                    li = ddlTeamAssignedTo.Items.FindByValue(objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default/HD_Default").Attributes("FUN_ASSIGNEDTO").Value)
                    If li IsNot Nothing Then
                        ddlTeamAssignedTo.SelectedValue = li.Value
                    End If
                End If
                If objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default/HD_Default").Attributes("CONTACT_TYPE_ID").Value <> "" Then
                    HdDefaultContactType.Value = objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default/HD_Default").Attributes("CONTACT_TYPE_ID").Value
                    li = ddlContactType.Items.FindByValue(objOutputXml.DocumentElement.SelectSingleNode("ETHelpDesk_Default/HD_Default").Attributes("CONTACT_TYPE_ID").Value)
                    If li IsNot Nothing Then
