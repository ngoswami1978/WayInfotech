...
Partial Class ETHelpDesk_HDUP_helpDeskFeedBack_new
    ...
    Sub checkSecurity()
        ...
        objSecurityXml.LoadXml(Session("Security"))
        If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
            If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='ET Feedback Action']").Count <> 0 Then
                strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='ET Feedback Action']").Attributes("Value").Value)
            End If
    ...
    Protected Sub theTabStrip_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles theTabStrip.ItemDataBound
        ...
        End If
        Button1.Attributes.Add("onclick", "return ColorMethodHelpDeskFeedBackNew('" & Button1.ClientID.ToString() & "',2,'" & e.Item.ItemIndex & "');")
    ...
    Private Sub ViewFeedbackDetails()
        ...
        Dim strBuildersupervisory As New StringBuilder
        Dim objSecurityXml As New XmlDocument
        objSecurityXml.LoadXml(Session("Security"))
        If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
            If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='ET Feedback Action Supervisory']").Count <> 0 Then
                strBuildersupervisory = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='ET Feedback Action Supervisory']").Attributes("Value").Value)
            Else
                strBuildersupervisory = objeAAMS.SecurityCheck(0)
            End If
        Else
            strBuildersupervisory = objeAAMS.SecurityCheck(31)
        End If
        '#############################
        ...
        '        Dim objhdFeedbackView As New AAMS.bizETrackerHelpDesk.bzFeedback
        Dim objhdFeedbackView As New AAMS.bizETrackerHelpDesk.bzFeedback
        ...
        If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
            ...
            With objOutputXml.DocumentElement.SelectSingleNode("FEEDBACK")
                ...
                'IntDatediff = DateDiff(DateInterval.Day, DateTime.Parse(txtFeedbkDt.Text.Trim), DateTime.Now)
                IntDatediff = CInt(.Attributes("NO_OF_DAYS").Value.Trim())
                ' End If
            ...
            For Each objNode In xNodeList
                ...
                Select Case objNode.Attributes("SUG_DEPT_ID").Value
                    Case "1"
                        ...
                        '###################
                        If strBuildersupervisory(2) = "0" Then
                            If IntDatediff > 7 Then
                                ddlHelpDeskAction.Enabled = False
                            End If
                        End If
                        '####################
                    ...
                    Case "2"
                        ...
                        If strBuildersupervisory(2) = "0" Then
                            If IntDatediff > 7 Then
                                ddlTechnicalHelpDeskAction.Enabled = False
                            End If
                        End If
                        '####################
                    ...
                    Case "3"
                        ...
                        If strBuildersupervisory(2) = "0" Then
                            If IntDatediff > 7 Then
                                ddlSalesHelpDeskAction.Enabled = False
                            End If
                        End If
                    ...
                    Case "4"
                        ...
                        If strBuildersupervisory(2) = "0" Then
                            If IntDatediff > 7 Then
                                ddlTrainingHelpDeskAction.Enabled = False
                            End If
                        End If
                        '####################
                    ...
                    Case "5"
                        ...
                        If strBuildersupervisory(2) = "0" Then
                            If IntDatediff > 7 Then
                                ddlProductHelpDeskAction.Enabled = False
                            End If
                        End If
                        '####################
                    Case "6"
                        ...
                        If strBuildersupervisory(2) = "0" Then
                            If IntDatediff > 7 Then
                                ddlCustomerFeedbackAction.Enabled = False
                            End If
                        End If
                        '####################
    ...
    Protected Sub txtSuggestionCustomerFeedback_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSuggestionCustomerFeedback.TextChanged
