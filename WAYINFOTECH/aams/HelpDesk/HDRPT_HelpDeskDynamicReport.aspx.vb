...
'########################################################################
Partial Class HelpDesk_HDRPT_HelpDeskDynamicReport
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If (Not IsPostBack) Then
                ...
                objeAAMS.BindDropDown(drpFollowup, "WorkOrderFollowUp", True, 3)
                ...
                If Session("LoginSession") IsNot Nothing Then
                    ...
                    If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                        ...
                        objNode = objOutputXml.DocumentElement.SelectSingleNode("FIELDDETAILS[@DM_FIELD_ID='AIRLINE_NAME']")
                        If objNode IsNot Nothing Then
                            If objNode.Attributes("SELECTED").Value = "True" Then
                                DivAirLine.Visible = True
                            End If
                        End If
    ...
    Sub SearchRecords()
        ...
        If Session("LoginSession") IsNot Nothing Then
            ...
            If DivAirLine.Visible = True Then
                If DlstAirLine.SelectedValue <> "" Then
            ...
            ' Here Back end Method Call
            'objOutputXml.Load("c:\DMreport.xml")
            ...
            Try
                objOutputXml.Save("c:\Admin\DM.xml")
            Catch ex As Exception

            End Try
            If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                ' ExportDataInCSVformat(objOutputXml)

                Session("HelpDeskDynamicReport") = objOutputXml.OuterXml
                Response.Redirect("../RPSR_ReportShow.aspx?Case=HelpDeskDynamicReport", False)
            Else
    ...
    Protected Sub BtnRequestForReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnRequestForReport.Click
        ...
        End Try
    End Sub
    Private Sub ExportDataInCSVformat(ByVal objOutputXml As XmlDocument)
        
        Dim FinalXml As New XmlDocument
        Try
            FinalXml.LoadXml("<HD_DMSEARCH_CALLREQUEST_OUTPUT><SEARCHDETAILS></SEARCHDETAILS></HD_DMSEARCH_CALLREQUEST_OUTPUT>")

            Dim objnode As XmlNode

            objnode = FinalXml.DocumentElement.SelectSingleNode("SEARCHDETAILS")

            For Each ObjXmlAttribute As XmlAttribute In objOutputXml.DocumentElement.SelectSingleNode("HEADER").Attributes
                Dim xmlAttr As XmlAttribute
                If ObjXmlAttribute.Value.ToString.Trim.Length > 0 Then
                    Dim Attrname As String = ObjXmlAttribute.Value.ToString
                    Attrname = Attrname.Replace(" ", "_")
                    If Attrname = "1A_Office" Then
                        Attrname = "A_Office"
                    End If

                    xmlAttr = FinalXml.CreateAttribute(Attrname)
                 
                    objnode.Attributes.Append(xmlAttr)
                End If

            Next


            If objOutputXml.DocumentElement.SelectNodes("SEARCHDESC[@Ltr!='']").Count > 0 And objOutputXml.DocumentElement.SelectNodes("SEARCHSOLN[@Ltr!='']").Count > 0 Then
                Dim xmlAttrDesc As XmlAttribute
                Dim xmlAttSoln As XmlAttribute
                xmlAttrDesc = FinalXml.CreateAttribute("Description")
                objnode.Attributes.Append(xmlAttrDesc)
                xmlAttSoln = FinalXml.CreateAttribute("Solution")
                objnode.Attributes.Append(xmlAttSoln)
            End If
            ...
            If objOutputXml.DocumentElement.SelectNodes("SEARCHDESC[@Ltr!='']").Count <= 0 And objOutputXml.DocumentElement.SelectNodes("SEARCHSOLN[@Ltr!='']").Count > 0 Then
                Dim xmlAttrDesc As XmlAttribute
                xmlAttrDesc = FinalXml.CreateAttribute("Solution")
                objnode.Attributes.Append(xmlAttrDesc)
            End If


            Dim objnodeClone As XmlNode = objnode.CloneNode(True)

            FinalXml.DocumentElement.RemoveChild(objnode)

            For Each objDetailxmlnode As XmlNode In objOutputXml.DocumentElement.SelectNodes("SEARCHDETAILS")
                Dim IntAttributesNo As Integer = 0
                For Each ObjXmlAttribute As XmlAttribute In objnodeClone.Attributes
                    If ObjXmlAttribute.Name <> "Description" And ObjXmlAttribute.Name <> "Solution" Then
                        ObjXmlAttribute.Value = objDetailxmlnode.Attributes(IntAttributesNo).Value
                        IntAttributesNo = IntAttributesNo + 1
                    End If
                Next

                Dim strdesc As String = ""
                Dim strsoln As String = ""
                Dim strFinaldesc As String = ""
                Dim strsFinaloln As String = ""
                For Each objDescxmlnode As XmlNode In objOutputXml.DocumentElement.SelectNodes("SEARCHDESC[@Ltr='" + objDetailxmlnode.Attributes("Ltr").Value + "']")
                    ' If strdesc.Trim.Length > 0 Then
                    strFinaldesc = "=========Logged By : " + objDescxmlnode.Attributes("LoggedBy").Value + "    Modified By : " + objDescxmlnode.Attributes("ModifiedBy").Value + "   DateTime : " + objDescxmlnode.Attributes("Datetime").Value + "  ========================================="
                    ' End If
                    strdesc = objDescxmlnode.Attributes("Desc").Value
                    '&#xD;&#xA;&#xD;&#xA
                    strdesc = strdesc.Replace(vbCrLf, " ")
                    strdesc = strdesc.Replace("&#xD;", " ")
                    strdesc = strdesc.Replace("&#xA;", " ")
                    strdesc = strdesc.Replace("&#xD;", " ")
                    strdesc = strdesc.Replace("&#xA;", " ")
                    strFinaldesc = strFinaldesc + strdesc

                Next
                If objnodeClone.Attributes("Description") IsNot Nothing Then
                    objnodeClone.Attributes("Description").Value = strFinaldesc
                End If


                For Each objSolnxmlnode As XmlNode In objOutputXml.DocumentElement.SelectNodes("SEARCHSOLN[@Ltr='" + objDetailxmlnode.Attributes("Ltr").Value + "']")

                    'If strsoln.Trim.Length > 0 Then
                    '    strsoln = strsoln + "=================================================="
                    'End If
                    strsFinaloln = strsFinaloln + "=========Logged By : " + objSolnxmlnode.Attributes("LoggedBy").Value + "    Modified By : " + objSolnxmlnode.Attributes("ModifiedBy").Value + "   DateTime : " + objSolnxmlnode.Attributes("Datetime").Value + "  ========================================="

                    strsoln = objSolnxmlnode.Attributes("Soln").Value
                    strsoln = strsoln.Replace(vbCrLf, " ")
                    strsoln = strsoln.Replace("&#xD;", " ")
                    strsoln = strsoln.Replace("&#xA;", " ")
                    strsoln = strsoln.Replace("&#xD;", " ")
                    strsoln = strsoln.Replace("&#xA;", " ")
                    strsFinaloln = strsFinaloln + strsoln

                Next
                If objnodeClone.Attributes("Solution") IsNot Nothing Then
                    objnodeClone.Attributes("Solution").Value = strsFinaloln
                End If

                FinalXml.DocumentElement.AppendChild(objnodeClone)
                objnodeClone = objnode.CloneNode(True)
            Next


            Dim strArray(FinalXml.DocumentElement("SEARCHDETAILS").Attributes.Count - 1) As String
            Dim intArray(FinalXml.DocumentElement("SEARCHDETAILS").Attributes.Count - 1) As Integer

            For k As Integer = 0 To intArray.Length - 1
                intArray(k) = k
                strArray(k) = FinalXml.DocumentElement("SEARCHDETAILS").Attributes(k).Name.Replace("_", " ")
            Next

            Try
                FinalXml.Save("c:\FinalDMreportxml.xml")
            Catch ex As Exception

            End Try

            objExport.ExportDetails(FinalXml, "SEARCHDETAILS", intArray, strArray, ExportExcel.ExportFormat.Excel, "DynamicReport.xls")
