'##########################################################################
'############   Page Name -- Popup_PUSR_ChallanStatusMRHistory      #######
'############   Date 26-March 2008  ####################################
'############   Developed By Abhishek  ####################################
'##########################################################################
Partial Class Popup_PUSR_ChallanStatusMRHistory
...
#End Region
    ...
    Sub LoadChallanStatusHistroy()
        ...
        Try
            ...
            objInputXml.LoadXml("<INV_GETMISCHISTORY_INPUT><ROWID/></INV_GETMISCHISTORY_INPUT>")
            objInputXml.DocumentElement.SelectSingleNode("ROWID").InnerText = Request.QueryString("RowId")
            'Here Back end Method Call
            objOutputXml = objbzChallan.GetMiscReplacementHistory(objInputXml)

            If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                objXmlReader = New XmlNodeReader(objOutputXml)
                ds.ReadXml(objXmlReader)
                grdChallanStatusMRHistory.DataSource = ds.Tables("MISCHISTORY")
                grdChallanStatusMRHistory.DataBind()
            Else
                grdChallanStatusMRHistory.DataSource = Nothing
                grdChallanStatusMRHistory.DataBind()
