...
Partial Class BirdresHelpDesk_HDSR_HelpDeskFeedback
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BR Feedback Action']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BR Feedback Action']").Attributes("Value").Value)
                End If
    ...
    Sub SearchRecords(ByVal Operation As Integer)
        ...
        objInputXml.LoadXml("<HD_SEARCHFEEDBACK_INPUT><LCode/><AGENCYNAME/><OfficeID/><FEEDBACK_ID/><ExecutiveName/><HD_RE_ID/><DATEFROM/><DATETO/><LOGGEDBY/><DEPT/><ASSIGNEDTO/><FEEDBACK_STATUS_ID/><ISCRITICAL/><PAGE_NO/> <PAGE_SIZE/><SORT_BY/><DESC/><Limited_To_Aoffice/><Limited_To_Region/><Limited_To_OwnAagency/><EmployeeID /></HD_SEARCHFEEDBACK_INPUT>")
    ...
    Sub DeleteRecords()
        Dim objInputXml, objOutputXml As New XmlDocument
        Try
            Dim objFbDele As New AAMS.bizBRHelpDesk.bzFeedback
...
#End Region
    ...
    Sub Export(ByVal objOutputXml As XmlDocument)
        ...
        '	          STATUS_PRODUCT=""/>
        Dim strArray() As String = {"FeedBack ID", "LTR No", "Agency Name", "Office Id", "Aoffice", "Date", "HelpDesk Status", "Techanical Status", "Sales Status", "Training Status", "Product Status"}
        Dim intArray() As Integer = {0, 1, 4, 7, 5, 6, 8, 9, 10, 11, 12}
