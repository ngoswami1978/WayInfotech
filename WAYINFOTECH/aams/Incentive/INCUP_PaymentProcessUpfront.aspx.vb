...
Imports System.Math
Partial Class Incentive_INCUP_PaymentProcessUpfront
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        txtLatestUpfontAmount.Attributes.Add("onblur", "validateDecimalValue('" + txtLatestUpfontAmount.ClientID + "')")
        txtPLBAmt.Attributes.Add("onblur", "validateDecimalValue('" + txtPLBAmt.ClientID + "')")

        HdPaidUpFrontAtthisLevel.Value = 0
        ...
        If Not Page.IsPostBack Then
            ...
            If Request.QueryString("OnlyShow") IsNot Nothing Then
                '  GvProcessPayment.Enabled = False
    ...
    Private Sub LoadData(ByVal strChainCode As String, ByVal strBcaseId As String)
        ...
        Try
            ...
            If objOutPutxml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                ...
                If ds.Tables("GROUPDETAILS") IsNot Nothing Then
                    ...
                    HdUpNoPayDone.Value = ds.Tables("GROUPDETAILS").Rows(0)("UPF_NO_OF_PAYMENT_DONE").ToString
                    HdOneTimeUpNoOfPay.Value = ds.Tables("GROUPDETAILS").Rows(0)("UPF_ONETIME_NO_OF_PAYMENT").ToString
                    HdOneTimeUpNoPayDone.Value = ds.Tables("GROUPDETAILS").Rows(0)("UPF_ONETIME_NO_OF_PAYMENT_DONE").ToString
                    If Request.QueryString("CurPayNo") IsNot Nothing Then
                        ' HdCurPayNo.Value = Val(Request.QueryString("CurPayNo").ToString)
                    End If
                ...
                Try
                    ...
                    txtPrevUpfrontAmount.Text = "0.00"
                    txtBalanceUpfrontAmount.Text = "0.00"
                    txtLatestUpfontAmount.Text = "0.00"
                    txtUpFrontAmount.Text = "0.00"
                    ...
                    Else
                        ...
                        'If hdUpFronttType.Value <> "1" Then
        ...
        'End If
        '@ Now Caculation for ----- Upfront Amount/Previous Upfront Amount/ Balance Upfront Amount/ Latest Upfront Amount When Business Is Changed (ie. if any change in likely window
        CalculateBalance()
        If hdFinallyApproved.Value.Trim.ToUpper = "TRUE" Then
            btnSave.Enabled = False
            BtnSendForApproval.Enabled = False
        End If
        HideUnhidePayableUpfrontAmount()
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If (IsValid) Then
            Try
                ...
                If hdUpFronttType.Value = "3" AndAlso hdPaymentType.Value.Trim.ToUpper = "U" Then
                    'If txtLatestUpfontAmount.Visible = True Then
                    '    If txtLatestUpfontAmount.ReadOnly = False Then
                    '        If HdYearAndSettleMent.Value.Trim.ToUpper = "FALSE" Then
                    '            If Val(txtLatestUpfontAmount.Text) <> Val(txtPrevUpfrontAmount.Text) Then
                    '                lblError.Text = "In case of Upfront Fixed deals Previous upfront amount and current upfront amount must be same."
                    '                Exit Sub
                    '            End If
                    '        End If
                    '    End If
                ...
                If txtLatestUpfontAmount.Visible = True Then
                    '@ Start Checking Decimal Value
                    Dim reg As Regex = New Regex("^[0-9.]+$")
                    If reg.IsMatch(txtLatestUpfontAmount.Text) Then
                        Dim countDecimalNo As Integer = 0
                        For i As Integer = 0 To txtLatestUpfontAmount.Text.Length - 1
                            If txtLatestUpfontAmount.Text.Chars(i) = "." Then
                                countDecimalNo = countDecimalNo + 1
                            End If
                        Next
                        If countDecimalNo > 1 Then
                            lblError.Text = "Only numeric with decimal is allowed."
                            Dim scriptManager As ScriptManager = scriptManager.GetCurrent(Me.Page)
                            scriptManager.SetFocus(txtLatestUpfontAmount.ClientID)
                            Exit Sub
                        End If
                    Else
                        lblError.Text = "Only numeric with decimal is allowed."
                ...
                If (Not Request.QueryString("BCaseID") = Nothing) Then
                    ...
                    objInputXml.DocumentElement.SelectSingleNode("BCDETAIL").Attributes("UPFRONT_AMOUNT").Value = txtUpFrontAmount.Text
                    objInputXml.DocumentElement.SelectSingleNode("BCDETAIL").Attributes("PREVIOUSUPFRONTAMOUNT").Value = txtPrevUpfrontAmount.Text
                    objInputXml.DocumentElement.SelectSingleNode("BCDETAIL").Attributes("BALANCEUPFRONTAMOUNT").Value = txtBalanceUpfrontAmount.Text
                    If txtLatestUpfontAmount.Visible = True Then
                        objInputXml.DocumentElement.SelectSingleNode("BCDETAIL").Attributes("NEXTUPFRONTAMOUNT").Value = txtLatestUpfontAmount.Text '@ Upfront Payment Now
                    End If
                    ...
                    '   QUALIAVG='' TotalQuali='' TotalQualiAfterExem=''
                    ...
                    objChildNode = objInputXml.DocumentElement.SelectSingleNode("CALCULATION/PRODTYPEDETAILS")
                    ...
                    Try
                        objOutputXml.Save("C:\Admin\UpdatePaymentProcessDetailsOut.xml")
                    Catch ex As Exception
                    End Try
                    If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                        lblError.Text = "Payment Updated Successfully."

                        HdPaidUpFrontAtthisLevel.Value = Val(txtLatestUpfontAmount.Text)
                        If Request.QueryString("Chain_Code") IsNot Nothing Then
                            hdEnChainCode.Value = Request.QueryString("Chain_Code")
                            hdChainCode.Value = objED.Decrypt(hdEnChainCode.Value)
                            If Request.QueryString("BCaseID") IsNot Nothing Then
    ...
    Protected Sub BtnSendForApproval_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSendForApproval.Click
        ...
        If ViewState("PaymentData") IsNot Nothing Then
            ...
            End If
            If txtLatestUpfontAmount.Visible = True Then
                If Val(objXmlOuputdoc.DocumentElement.SelectSingleNode("GROUPDETAILS").Attributes("NEXTUPFRONTAMOUNT").Value) <> Val(txtLatestUpfontAmount.Text) Then
                    mdlPopUpExt.Show()
                    Exit Sub
                End If
            End If
    ...
    Private Sub LoadDataByPayId(ByVal strPayId As String)
        ...
        Try
            ...
            '  objOutPutxml.Load("C:\AAMSXml\BCPaymentProcessViewOut.xml")
            Try
                objOutPutxml.Save("C:\Admin\BCPaymentProcessViewOutput.xml")
            Catch ex As Exception
            End Try
            If objOutPutxml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                ViewState("PaymentData") = objOutPutxml.OuterXml
    ...
    Protected Sub txtRate_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            '##############################################################################################
            '@Start of  New Added Code if Revised rate is different then standard rate than RateRem is mandatory 20/10/10
            ...
            '    End If
            'End If
            '@ End of New Added Code if Revised rate is different then standard rate than RateRem is mandatory
            '##############################################################################################
    ...
    Protected Sub txtExumption_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Dim strROIRate As String = ""
            For Each row As GridViewRow In GvProcessPayment.Rows
                Dim strCaseName As String = CType(row.FindControl("lblProductivity"), System.Web.UI.WebControls.Label).Text.Trim().ToUpper()
                If strCaseName.ToUpper() = "ROI" Then
                    strROIRate = CType(row.FindControl("txtExumption"), TextBox).Text.Trim()
                End If
            Next
            ...
            If CType(gvrow.FindControl("lblProductivity"), System.Web.UI.WebControls.Label).Text.Trim().ToUpper() = "HL" Then
                For Each row As GridViewRow In GvProcessPayment.Rows
                    ...
                    If strCaseName.ToUpper() <> "HL" And strCaseName.ToUpper() <> "ROI" Then
                        ...
                        strHLCalculation = Val(CalculateGridData(hdRowID.Value.Trim(), txtExp1, "HL"))
            ...
            If CType(gvrow.FindControl("lblProductivity"), System.Web.UI.WebControls.Label).Text.Trim().ToUpper() = "ROI" Then
                For Each row As GridViewRow In GvProcessPayment.Rows
                    ...
                    If strCaseName.ToUpper() <> "HL" And strCaseName.ToUpper() <> "ROI" Then
                        ...
                        txtActulaSegment1.Text = dbTotal
                        Dim strHLCalculation As Double = 0
                        strHLCalculation = Val(CalculateGridData(hdRowID.Value.Trim(), Val(strHLRate), "HL"))
                        If strHLCalculation <> 0 Then
                            txtActulaSegment1.Text = (dbTotal + Val(strHLCalculation)).ToString()
                        End If
                        Dim dbROI As Double = 0
    ...
    Protected Sub txtLatestUpfontAmount_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            HdPaidUpFrontAtthisLevel.Value = Val(txtLatestUpfontAmount.Text)
            ''@ Start Checking Decimal Value
            'Dim reg As Regex = New Regex("^[0-9.]+$")
    ...
    Protected Sub txtPLBAmt_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            '@ Start of Code Addedd  on 03/09/10
            CalculateBalance()
            HideUnhidePayableUpfrontAmount()
        Catch ex As Exception
    ...
    Private Function CalculateGridData(ByVal strROWid As String, ByVal intRate As Double, ByVal strType As String) As Double
            ...
            If strType.ToUpper() = "HL" Then
                For Each xNode As XmlNode In objXmlOuputdoc.DocumentElement.SelectNodes("PRODTYPE[@ROWID='" + strROWid + "' and @HL='TRUE']")
                    If Val(xNode.Attributes("STANDARSEGMENT").Value) <> 0 Then
                        ...
                        'dbTotal = dbTotal + System.Math.Ceiling(((Val(xNode.Attributes("STANDARSEGMENT").Value) * intRate)) / 100)
        ...
        End Try
    End Function
    Private Sub EnableDisableGeneratePaymentAdvice()
        ''@ In Case of ( FinnalySettlement and Endyearsettlement) Balance( Business- (Current Upfront + PLB Amount) )is less than zero Payment advice  can't be genereted
        'If HdYearAndSettleMent.Value.Trim.ToUpper = "TRUE" Or hdEndSettlement.Value.Trim.ToUpper = "TRUE" Then
        '    Dim DblBusinessAmout As Double = 0
        '    If GvProcessPayment.Rows.Count > 0 Then
        '        Dim txtGrandFinalAmount As TextBox = CType(GvProcessPayment.FooterRow.FindControl("txtGrandFinalAmount"), TextBox)
        '        If Val(txtGrandFinalAmount.Text) - (Val(txtLatestUpfontAmount.Text) + Val(txtPLBAmt.Text)) < 0 Then
        '            BtnSendForApproval.Enabled = False
        '        End If
        '    End If
        'Else
        '    '@ In Case of except ( FinnalySettlement and Endyearsettlement)Balance (previous upfront- Current Upfront)is less than zero Payment advice  can't be genereted
    ...
    Private Sub HideUnhidePayableUpfrontAmount()

        If HdPLBCycle.Value.Trim.ToUpper = "TRUE" Then '@ In Case of PLB Payment
            lblLatestUpfontAmount.Visible = False
            trLatestUpfontAmount.Visible = False
            txtLatestUpfontAmount.Visible = False
            ...
            Else
                ...
                If Val(txtBalanceUpfrontAmount.Text) < 0 Then
                    ...
                    If hdFirstTime.Value.Trim.ToString.ToUpper <> "TRUE" Then
                        BtnSendForApproval.Enabled = False
                    Else
                        If HdPaymentId.Value.Trim.Length <= 0 Or HdPACreated.Value.ToUpper = "TRUE" Then
                            BtnSendForApproval.Enabled = False
                        End If
                    End If
                Else
                ...
                If hdUpFronttType.Value = "1" AndAlso hdPaymentType.Value.Trim.ToUpper = "U" Then 'In Case of OneTime Upfront  
                    If Val(HdOneTimeUpNoOfPay.Value) > 0 Then
                        If Val(HdOneTimeUpNoPayDone.Value) <= Val(HdOneTimeUpNoOfPay.Value) Then
                            If Val(txtBalanceUpfrontAmount.Text) >= 0 Then
                                lblLatestUpfontAmount.Visible = True
                                trLatestUpfontAmount.Visible = True
                                txtLatestUpfontAmount.Visible = True
                    ...
                    End If
                End If
                If hdUpFronttType.Value = "3" AndAlso hdPaymentType.Value.Trim.ToUpper = "U" Then 'In Case of Fixed Upfront  
                    If Val(HdUpNoPay.Value) > 0 Then
                        If Val(HdUpNoPayDone.Value) <= Val(HdUpNoPay.Value) Then
                            If Val(txtBalanceUpfrontAmount.Text) >= 0 Then
                                lblLatestUpfontAmount.Visible = True
                                trLatestUpfontAmount.Visible = True
                                txtLatestUpfontAmount.Visible = True
                ...
                If hdUpFronttType.Value = "2" AndAlso hdPaymentType.Value.Trim.ToUpper = "U" Then 'In Case of Repllinishible Upfront  
                    If Val(txtBalanceUpfrontAmount.Text) >= 0 Then
                        lblLatestUpfontAmount.Visible = True
                        trLatestUpfontAmount.Visible = True
                        txtLatestUpfontAmount.Visible = True
                ...
                If txtLatestUpfontAmount.Visible = True Then
                    txtLatestUpfontAmount.ReadOnly = False
                    txtLatestUpfontAmount.CssClass = "textboxbold right"
                End If
            End If
                    ...
                    If objOutPutxml.DocumentElement.SelectNodes("BC_PLAN/CASE/NIDT_FIELDS_ID[@NIDT_FIELDS_IDNAME='" + hdProductivityId.Value + "']").Count > 0 Then
                        ...
                        For Each objxmlnode As XmlNode In objFinalXmldocument.DocumentElement.SelectNodes("CASE/PLAN_DETAILS")
                            ...
                            If Val(strCalCulatedSegmentByBiilingPeriod) >= Val(objxmlnode.Attributes("SLABS_START").Value) And Val(strCalCulatedSegmentByBiilingPeriod) <= Val(objxmlnode.Attributes("SLABS_END").Value) Then
                                ' If Val(txtCalCulatedSegment.Text) >= Val(objxmlnode.Attributes("SLABS_START").Value) And Val(txtCalCulatedSegment.Text) <= Val(objxmlnode.Attributes("SLABS_END").Value) Then
        ...
        If (IsValid) Then
            Try
                ...
                If (Not Request.QueryString("BCaseID") = Nothing) Then
                    ...
                    objInputXml.DocumentElement.SelectSingleNode("BCDETAIL").Attributes("TOTAL_AMT").Value = 0
                    'PREVPAYMENT_ID=''
                    ' Here Back end Method Call
                    'Code Added by Mukund for Importing Rows of View
                    Dim xmlView As New XmlDocument
