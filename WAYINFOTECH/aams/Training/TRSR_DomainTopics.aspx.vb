...
Partial Class Training_TRSR_DomainTopics
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		Try
            ...
            If Not IsPostBack Then
                ...
                'Sir has asked to remove it  date 11 July 08
                ' instead of removing i m setting its visible property to false
                'objeAAMS.BindDropDown(ddlDomain, "DOMAIN", True, 3)
                ddlCourse.Focus()
...
#Region "btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click"
	Protected Sub btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
		Response.Redirect("TRSR_DomainTopics.aspx")
	End Sub
#End Region
...
#Region "BindData()"
    Private Sub BindData(ByVal Operation As Integer)
        ...
        Try
            ...
            'objInputXml.DocumentElement.SelectSingleNode("TR_VALTOPICDOM_ID").InnerText = ddlDomain.SelectedValue
            ...
            If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                ...
                If ds.Tables("TOPIC").Rows.Count <> 0 Then
                    gvDomain.DataSource = ds.Tables("TOPIC")
                    gvDomain.DataBind()
                    ...
                    Else
                        lnkPrev.Visible = True
                    End If
                    hdRecordOnCurrentPage.Value = ds.Tables("TOPIC").Rows.Count.ToString
                    ...
                    Select Case ViewState("SortName").ToString
                        ...
                        Case "TR_COURSE_NAME"
                            ...
                            '    Select Case ViewState("Desc")
                            '        Case "FALSE"
                            '            gvDomain.HeaderRow.Cells(2).Controls.Add(imgUp)
                            '        Case "TRUE"
                            '            gvDomain.HeaderRow.Cells(2).Controls.Add(imgDown)
...
#Region "DeleteRecords()"
	Private Sub DeleteRecords()
		Try
			...
			Dim objbzTopic As New AAMS.bizTraining.bzTopic
            If hdCourseId.Value <> "" Then
                objInputXml.LoadXml("<TR_DELETE_TOPIC_INPUT><TOPIC TR_COURSE_ID=''/></TR_DELETE_TOPIC_INPUT> ")
                objInputXml.DocumentElement.SelectSingleNode("TOPIC").Attributes("TR_COURSE_ID").Value = hdCourseId.Value
                hdCourseId.Value = ""
                objOutputXml = objbzTopic.DeleteAll(objInputXml)
                BindData(PageOperation.Search)
                If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
...
#Region "gvDomain_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvDomain.RowDataBound"
	Protected Sub gvDomain_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvDomain.RowDataBound
		Try
            ...
            Dim hdCourse As HiddenField
			Dim btnEdit As LinkButton
            hdDomain = CType(e.Row.FindControl("hdDomain"), HiddenField)
            hdCourse = CType(e.Row.FindControl("hdCourse"), HiddenField)
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                ...
                Else
                    btnDelete.Attributes.Add("OnClick", "javascript:return Delete('" + hdCourse.Value + "');")
                End If
                'If strBuilder(2) = "0" Then
                '    btnEdit.Enabled = False
                'Else
                '    btnEdit.Attributes.Add("OnClick", "javascript:return Edit('" + hdCourse.Value + "');")
                'End If
                btnEdit.Attributes.Add("OnClick", "javascript:return Edit('" + objED.Encrypt(hdCourse.Value) + "');")
...
#End Region
    ...
    Sub Export(ByVal objOutputXml As XmlDocument)
        ...
        Dim strArray() As String = {"Domain", "Course", "Topic"}
        Dim intArray() As Integer = {2, 5, 3}
        objExport.ExportDetails(objOutputXml, "TOPIC", intArray, strArray, ExportExcel.ExportFormat.Excel, "exportDOMAINTOPICS.xls")
    End Sub
    'End Code For Export
End Class
