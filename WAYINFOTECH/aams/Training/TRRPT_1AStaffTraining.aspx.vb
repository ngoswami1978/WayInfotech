...
Partial Class Training_TRRPT_1AStaffTraining
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='1A Staff Training']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='1A Staff Training']").Attributes("Value").Value)
...
#End Region
    ...
    Private Sub StaffTrainingReport()
        Dim objInputXml, objOutputXml As New XmlDocument
        Dim objSecurityXml As New XmlDocument
        Dim objbzTraining As New AAMS.bizTraining.bzTraining

        Try
            objInputXml.LoadXml("<TR_RPT_1A_STAFF_TRAINING_INPUT><AgencyType/><LCODE/><AGENCYNAME/><GROUPDATA/><COURSEID/><LOCATION_NAME/><TRAINER1/><TRAINER2/><AOFFICE/><AGENCYSTAFFID/><AGENCYSTAFFNAME/><REGION/><DATE_TYPE_FROM/><DATE_TYPE_TO/><DATE_TYPE/><COURSE_STARTDATE_FROM/><COURSE_STARTDATE_TO/><COURSE_ENDDATE_FROM/><COURSE_ENDDATE_TO/><EMPLOYEE_ID/><Limited_To_Aoffice/><Limited_To_Region/><Limited_To_OwnAgency/><RESP_1A/></TR_RPT_1A_STAFF_TRAINING_INPUT>")
            With objInputXml.DocumentElement
                ...
                .SelectSingleNode("GROUPDATA").InnerText = ""
                ...
                End If
                .SelectSingleNode("DATE_TYPE_FROM").InnerText = objeAAMS.ConvertTextDate(txtStartDateFrom.Text.Trim())
                .SelectSingleNode("DATE_TYPE_TO").InnerText = objeAAMS.ConvertTextDate(txtStartDateTo.Text.Trim())
                .SelectSingleNode("DATE_TYPE").InnerText = ddlDateType.SelectedValue
            ...
            End With
            Session("1aStaffTraining") = objInputXml.OuterXml
            Response.Redirect("../RPSR_ReportShow.aspx?Case=1aStaffTrainging")
        Catch ex As Exception
            lblError.Text = ex.Message.ToString
        Finally
            objInputXml = Nothing
    ...
    Protected Sub btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
        Response.Redirect("TRRPT_1AStaffTraining.aspx")
    End Sub
End Class
