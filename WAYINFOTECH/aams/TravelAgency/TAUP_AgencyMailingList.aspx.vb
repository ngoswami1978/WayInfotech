Imports System.Xml
Imports System.Data
Partial Class TravelAgency_TAUP_AgencyMailingList
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim strEmailList As String = ""
        btnOrder.CssClass = "headingtabactive"
        Try
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='OrderSendMail']").Count <> 0 Then
                    ...
                    If strBuilder(1) = "0" Then
                        ...
                        ' btnSendMail.Attributes.Add("onclick", "return javascript:<a href=#></a>")
            ...
            If (Not IsPostBack) Then
                ...
                If Request.QueryString("Action") IsNot Nothing Then
                    ...
                    If Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "U" Or Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "US" Then
                        ...
                        objInputOrderXml.DocumentElement.SelectSingleNode("ORDERID").InnerText = Request.QueryString("Action").ToString().Split("|").GetValue(1).ToString()
                        hdOrderId.Value = Request.QueryString("Action").ToString().Split("|").GetValue(1).ToString()
    ...
    Protected Sub btnOrder_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOrder.Click
        Try
            Response.Redirect("TAUP_AgencyOrder.aspx?Id=7&EditOrderId=" + Request.QueryString("Action").ToString().Split("|").GetValue(1).ToString(), False)
        Catch ex As Exception
            lblError.Text = ex.Message
        End Try
    End Sub
    ...
    Protected Sub btnMail_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnMail.Click
        Try
            ...
            If Request.QueryString("Action") IsNot Nothing Then
                If Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "U" Or Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "US" Then
                    Response.Redirect("TAUP_AgencyMailingList.aspx?Id=7&Action=U|" + Request.QueryString("Action").ToString().Split("|").GetValue(1).ToString(), False)
                End If
            End If
