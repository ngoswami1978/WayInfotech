...
Partial Class BOHelpDesk_HDUP_LinkedLTR
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Linked HD Call']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Linked HD Call']").Attributes("Value").Value)
                End If
                        ...
                        If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                            ...
                            If hdTabType.Value <> "" Then
                                If (hdTabType.Value = 4 And hdPageStatus.Value.ToUpper = "TECHNICAL") Or (hdTabType.Value = 5 And hdPageStatus.Value.ToUpper = "FUNCTIONAL") Then
                                    Response.Redirect("BOHDUP_helpDeskFeedBack.aspx?Action=U&LCode=" + hdEnPageLCode.Value + "&HD_RE_ID=" + hdEnPageHD_RE_ID.Value + "&strStatus=" + hdEnPageStatus.Value + "&TabType=" + hdTabType.Value)
                                End If
                                ...
                                Response.Redirect("BOHDUP_CallLog.aspx?Action=U&LCode=" + hdEnPageLCode.Value + "&HD_RE_ID=" + hdEnPageHD_RE_ID.Value + "&strStatus=" + hdEnPageStatus.Value + "&TabType=" + hdTabType.Value)
                            End If
