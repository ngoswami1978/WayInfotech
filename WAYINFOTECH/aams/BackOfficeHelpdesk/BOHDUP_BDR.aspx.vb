...
Imports System.Data
#End Region
Partial Class BOHelpDesk_HDUP_BDR
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO BDR Letter Report']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO BDR Letter Report']").Attributes("Value").Value)
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO BDRLetter']").Count <> 0 Then
                    ...
                    '@ Added By abhishek on 12/04/2010
                    If strBuilder(1) = "0" Then
                        btnNewBdr.Enabled = False
                    End If
                    '@ Added By abhishek on 12/04/2010
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Log HD Call']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Log HD Call']").Attributes("Value").Value)
            ...
            End If
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO BDR History']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO BDR History']").Attributes("Value").Value)
                    If strBuilder(0) = "0" Then
                ...
                If strBuilder(0) = "1" Then
                    If (Request.QueryString("Action") = "U") Then
                        If (Request.QueryString("HD_RE_BDR_ID") IsNot Nothing) Then
                            ...
                            hdbdrLetterId.Value = hdEnbdrLetterId.Value ' objED.Decrypt(hdEnbdrLetterId.Value) 'Rakesh 2 Aug 08 Added
                            btnHistory.Disabled = False
                            btnHistory.Attributes.Add("onclick", "return PopupHistoryPage(" & "'" & objED.Encrypt(hdEnbdrLetterId.Value) & "'" & ");")
                        Else
                            btnHistory.Disabled = True
                        End If
                    Else
                ...
                If (Request.QueryString("Action") IsNot Nothing) Then
                    If (Request.QueryString("Action") = "I") Then
                        ...
                        If (Request.QueryString("LCode") IsNot Nothing) Then
                            ...
                            objInputXml.DocumentElement.SelectSingleNode("LOCATION_CODE").InnerText = hdLcode.Value 'Rakesh 2 Aug 08 Added
            ...
            If Not Page.IsPostBack Then                '
                ...
                If (Request.QueryString("Action") = "U") Then
                    ...
                    If Session("Msg") IsNot Nothing Then
                        If (Session("Msg").ToString = "U") Then
                            lblError.Text = objeAAMSMessage.messUpdate
                        End If
                        If (Session("Msg").ToString = "A") Then
                            lblError.Text = objeAAMSMessage.messInsert
                        End If
                        Session("Msg") = Nothing
                    End If
            ...
            '@ Start of Code added By Abhishek on 12/04/2010 for MultiBDRNavigation
            'If Not IsPostBack Then
            '    If (Request.QueryString("ReqId") IsNot Nothing) Then
            '        If (Request.QueryString("Action") IsNot Nothing) Then
            '            If (Request.QueryString("Action") = "U") Then
            '                If hdbdrLetterId.Value <> "" Then
            '                    BindListForBdr(objED.Decrypt(Request.QueryString("ReqId")), hdbdrLetterId.Value)
            '                End If
            ...
            If Not IsPostBack Then
                If (txtLtrNo.Text.Trim.Length > 0) Then
                    If (Request.QueryString("Action") IsNot Nothing) Then
                        If (Request.QueryString("Action") = "U") Then
                            If hdbdrLetterId.Value <> "" Then
                                BindListForBdr(txtLtrNo.Text.Trim, hdbdrLetterId.Value)
                            End If
                ...
                End If
            End If
            '@ End of  Code added By Abhishek on 12/04/2010 for MultiBDRNavigation
    ...
    Sub ViewBdrLetter()
        Try
            If (Not Request.QueryString("Action") = Nothing) Then
                ...
                Dim objbzbzRequestBDR As New AAMS.bizBOHelpDesk.bzRequestBDR
...
#Region "Call BindBdrLetterData For Binding Data in Controls"
    Sub BindBdrLetterData(ByVal objOutputXml As XmlDocument)
        ...
        Try
            ...
            txtFax.Text = objOutputXml.DocumentElement.SelectSingleNode("BDRLetter").Attributes("FAX").InnerText
            ...
            hdEnbdrLetterId.Value = txtBDrId.Text ' objED.Encrypt(txtBDrId.Text) 'Rakesh 2 Aug 08 Added
                ...
                If (Not Request.QueryString("Action") = Nothing) Then
                    ...
                    If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                        ...
                        If (Request.QueryString("Action") = "U") Then
                            ...
                            hdEnbdrLetterId.Value = strbdrLetterId ' objED.Encrypt(strbdrLetterId) 'Rakesh 2 Aug 08 Added
                            If Request.QueryString("Popup") Is Nothing Then
                                ...
                                Dim strHD_RE_ID As String = ""
                                strHD_RE_ID = objED.Encrypt(txtLtrNo.Text)
                                Session("Msg") = "U"
                                Response.Redirect("BOHDUP_BDR.aspx?Msg=U&Action=U&HD_RE_BDR_ID=" & hdEnbdrLetterId.Value, False) 'Rakesh 2 Aug 08 inplace of above line 
                            ...
                            Else
                                ...
                                strHD_RE_ID = objED.Encrypt(txtLtrNo.Text)
                                Session("Msg") = "U"
                                Response.Redirect("BOHDUP_BDR.aspx?Popup=T&Msg=U&Action=U&HD_RE_BDR_ID=" & hdEnbdrLetterId.Value & "&ReqID=" & strHD_RE_ID, False) 'Rakesh 2 Aug 08 inplace of above line
                        ...
                        If (Request.QueryString("Action") = "I") Then
                            ...
                            strHD_RE_ID = objED.Encrypt(txtLtrNo.Text)
                            Session("Msg") = "A"
                            Response.Redirect("BOHDUP_BDR.aspx?Popup=T&Msg=A&Action=U&HD_RE_BDR_ID=" & hdEnbdrLetterId.Value & "&ReqID=" & strHD_RE_ID, False)
...
#Region "btnPrint_ClickEvent"
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                Session("eBOBRDLetterRpt") = objOutputXml.OuterXml
                Response.Redirect("../RPSR_ReportShow.aspx?Case=BOBRDLetter", False)
            Else
                lblError.Text = objOutputXml.DocumentElement.SelectSingleNode("Errors/Error").Attributes("Description").Value
            End If
        Catch ex As Exception
...
#End Region
    ...
    Private Sub UpdateTemplate(ByRef objUpdateInputXml As XmlDocument)
        ...
        objInputAoffice.DocumentElement.SelectSingleNode("Aoffice").InnerText = strAoffice
        'Here Back end Method Call
    ...
    Protected Sub btnLtr_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLTR.Click
        If Request.QueryString("Popup") Is Nothing Then
            Response.Redirect("BOHDUP_CallLog.aspx?Action=U&LCode=" + hdEnLcode.Value + "&HD_RE_ID=" + hdEnRequestID.Value)
        End If
    End Sub
...
#Region "Code for Paging "
    ...
    Private Sub BindListForBdr(ByVal strReqId As String, ByVal strBdrId As String)
        Dim objInputXml, objOutputXml As New XmlDocument
        ...
        objInputXml.DocumentElement.SelectSingleNode("HD_RE_ID").InnerText = strReqId
        objOutputXml = objbzRequestBDR.ListBDR(objInputXml)
        If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then

            HdBdrIdXml.Value = objOutputXml.OuterXml
            objXmlReader = New XmlNodeReader(objOutputXml)
            ds = New DataSet
            ds.ReadXml(objXmlReader)
            Dim strTotalRecord As String = "0"
            strTotalRecord = ds.Tables("BDR").Rows.Count
            '<HD_LISTBDR_OUTPUT><BDR HD_RE_BDR_ID=''
            ' HD_RE_BDR_TICKETS=''/>
            '<Default/><LCODE/>
            '<Errors Status=''>
            '<Error Code='' Description='' /></Errors></HD_LISTBDR_OUTPUT>
            If strTotalRecord > 0 Then
                BindControlsForNavigation(strTotalRecord, strBdrId, ds)
            Else
    ...
    Private Sub BindControlsForNavigation(ByVal TotalNo As String, ByVal strBdrId As String, ByVal dset As DataSet)
        ...
        If count <> ddlPageNumber.Items.Count Then
            ...
            'Next
            For i As Integer = 1 To dset.Tables("BDR").Rows.Count
                'ddlPageNumber.Items.Add(i.ToString)
                ddlPageNumber.Items.Insert(i - 1, New ListItem(i.ToString, dset.Tables("BDR").Rows(i - 1)("HD_RE_BDR_ID").ToString))
            Next
        End If
        ...
        hdbdrLetterId.Value = selectedValue
    ...
    Protected Sub lnkPrev_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkPrev.Click
        Try
            If ddlPageNumber.SelectedItem.Text <> "1" Then
                '  ddlPageNumber.SelectedItem.Text = (CInt(ddlPageNumber.SelectedItem.Text) - 1).ToString
                ...
                hdbdrLetterId.Value = ddlPageNumber.SelectedValue
...
#End Region
    ...
    Protected Sub btnNewBdr_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNewBdr.Click
        Dim strLCode As String = ""
        Dim strHD_RE_ID As String = ""
        Dim requestType As String = ""
        Try
            If txtLtrNo.Text.ToString.Length > 0 Then
                If hdLcode.Value <> "" Then
                    strLCode = objED.Encrypt(hdLcode.Value.Trim)
                    If (hdCallCategoryName.Value <> "") Then
                        requestType = hdCallCategoryName.Value

                        strHD_RE_ID = objED.Encrypt(txtLtrNo.Text) ' Request.QueryString("ReqID").ToString
                        Response.Redirect("BOHDUP_BDR.aspx?Popup=T&Action=I&ReqID=" + strHD_RE_ID + "&LCode=" + strLCode + "&requestType=" + requestType, False)

                        'If (Request.QueryString("ReqID") IsNot Nothing) Then
                        '    strHD_RE_ID = Request.QueryString("ReqID").ToString
                        '    Response.Redirect("HDUP_BDR.aspx?Popup=T&Action=I&ReqID=" + strHD_RE_ID + "&LCode=" + strLCode + "&requestType=" + requestType, False)
                        'End If
                    End If
                End If
            Else
