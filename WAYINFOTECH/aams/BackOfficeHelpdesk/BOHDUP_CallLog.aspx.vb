...
Partial Class BOHelpDesk_HDUP_CallLog
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            'ddlTeamAssignedTo.Attributes.Add("OnChange", "return ClearAssignedDatetime();")
            'txtDateAssigned.Attributes.Add("OnChange", "return GetLoggedDateTime();")
            ...
            If Not Page.IsPostBack Then
                ...
                If Request.QueryString("QueryGroup") IsNot Nothing Then
                    ...
                    If Request.QueryString("Popup") Is Nothing Then
                        Response.Redirect("BOHDUP_helpDeskFeedBack.aspx?" + strQueryString)
                    Else

                        Response.Redirect("BOHDUP_helpDeskFeedBack.aspx?Popup=p" + strQueryString)
                    End If
                ...
                If Request.QueryString("MULTIHD_RE_ID") Is Nothing Then
                    If Not Request.QueryString("strStatus") Is Nothing Then
                        ...
                        If EnstrStatus.ToUpper = "TECHNICAL" Then
                            Dim strQueryString As String = Request.QueryString.ToString
                            Response.Redirect("BOTCUP_CallLog.aspx?" + strQueryString)
                        End If
                    End If
                ...
                If Not Request.QueryString("LCode") Is Nothing And Request.QueryString("MULTIHD_RE_ID") Is Nothing Then
                    ...
                    If Request.QueryString("HD_RE_ID") Is Nothing Then
                        AgencyView()
                        '@ Aded Code For Relog on 28/04/2011
                        If txtLoggedDate.Text.Trim.Length <= 0 Then
                            txtLoggedDate.Text = DateAndTime.Now.ToString("dd/MM/yyyy HH:mm:ss")
                            hdCallSatrtDateTimeInitialy.Value = DateAndTime.Now.ToString("dd/MM/yyyy HH:mm:ss")
                        End If
                        AutoCompleteExtenderForCaller.ContextKey = objED.Decrypt(Request.QueryString("LCode").ToString) + "|" + txtOfficeId.Text
                        '@ Aded Code For Relog on 28/04/2011
                    End If
                End If
    ...
    Sub checkSecurity()
        ...
        If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
            ...
            If strBuilderFunctional(1) = "0" And strBuilderFunctional(2) <> "0" And (hdPageHD_RE_ID.Value <> "" Or Request.QueryString("HD_RE_ID") IsNot Nothing) Then
                btnSave.Enabled = True
            End If
            If objeAAMS.ReturnViewPermission(Session("Security"), "BO Feedback Action") = "0" Then
                btnFeedBack.Enabled = False
            End If
        Else
    ...
    Sub LoadAllControl()
        Try
            ...
            objeAAMS.BindDropDown(ddlQuerySubGroup, "BOQuerySubGroup", True, 1)
            objeAAMS.BindDropDown(ddlQueryStatus, "BOQueryStatusWithClosed", True, 1)
            objeAAMS.BindDropDown(ddlPriority, "BOQUERYPRIORITY", True, 1)
            objeAAMS.BindDropDown(ddlTeamAssignedTo, "BOTEAM", True, 1)
            objeAAMS.BindDropDown(ddlContactType, "BOCONTACTTYPE", True, 1)
            objeAAMS.BindDropDown(ddlMode, "BOCONTACTTYPE", True, 2)
            objeAAMS.BindDropDown(ddlDisposition, "BODISPOSITION", True, 2)
            ...
            Dim objCallCategory1 As New AAMS.bizBOHelpDesk.bzDisposition
            Dim objSecurityXml As New XmlDocument
    ...
    Protected Sub theTabStrip_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles theTabStrip.ItemDataBound
        ...
        If Session("Security") IsNot Nothing Then
            ...
            strLogHDCallPermission = objeAAMS.ReturnViewPermission(Session("Security"), "BO Log HD Call")
            Select Case strButtonText
                ...
                Case "Follow Up"
                    ...
                    End If
                Case "Linked LTR"
                    If objeAAMS.ReturnViewPermission(Session("Security"), "BO Linked HD Call") = "0" Then
                        Button1.CssClass = "displayNone"
                    End If
    ...
    Sub SaveMultiLTR(ByVal Operation As Integer)
        ...
        Try
            ...
            With objInputXml.DocumentElement.SelectSingleNode("CALLREQUEST")
                ...
                '    strFullFileName = Server.MapPath("../BackOfficeHelpDeskFiles/") + strFileName
                '    If strFilePath <> "" Then
                ...
                .Attributes("AssignedDatetime").Value = objeAAMS.GetDateFormat(txtDateAssigned.Text.Trim, "dd/MM/yyyy HH:mm:ss", "yyyy-MM-dd HH:mm:ss", "/")
    ...
    Sub SaveInformation(ByVal Operation As Integer)
        ...
        Try
            ...
            With objInputXml.DocumentElement.SelectSingleNode("CALLREQUEST")
                ...
                '            Dim dtDays As TimeSpan = dtCloseDate - dtGiven
                '            Dim dtcount As Integer = DateDiff(DateInterval.DayOfYear, dtCloseDate, dtGiven)
                '            totalHours = dtDays.Days * 24
                ...
                '        Dim dtCurrentDate As Date = DateTime.Now.ToString("dd/MM/yyyy")

                '        Dim dtDays As TimeSpan = dtCurrentDate - dtGiven
                '        totalHours = dtDays.Days * 24
                ...
                'Dim dtNewTimeSpan As New TimeSpan(totalHours, 0, 0)
                'resultTime = resultTime.Add(dtNewTimeSpan)

                'If resultTime.TotalHours > 0 Then
                '    If resultTime.TotalHours < 10 Then
                '        strCallDuration = "0" & resultTime.TotalHours.ToString("0")
                '    Else
                '        strCallDuration = resultTime.TotalHours.ToString("0")
                '    End If
                'Else
                '    strCallDuration = "00"
                'End If
    ...
    Protected Sub btnReset_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
        Try
            ...
            Else
                Dim strQueryString As String = Request.QueryString.ToString
                Response.Redirect("BOHDUP_CallLog.aspx?" + strQueryString)
            End If
    ...
    Sub FillPreDesc(ByVal strSubcategoryId As String)
        Dim objInputXml, objOutputXml As New XmlDocument
        Dim objbzCallSubCategory As New AAMS.bizBOHelpDesk.bzCallSubCategory
        Try
    ...
    Protected Sub rdTechanical_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rdTechanical.CheckedChanged
        If hdPageStatus.Value = "I" Or hdPageStatus.Value = "" Then
            ...
            If hdCallAgencyName.Value <> "" Then
                ...
                'End If
                Response.Redirect("BOTCUP_Calllog.aspx?LCodeTemp=" + hdEnCallAgencyName_LCODE.Value)
            Else
                Response.Redirect("BOTCUP_Calllog.aspx?Action=I") ' + hdQueryString.Value)
            End If
        Else
            Response.Redirect("BOTCUP_Calllog.aspx?" + hdQueryString.Value)
        End If
    End Sub
    ...
    Sub fillDropDownList()
        ...
        Dim objPTR As New AAMS.bizBOHelpDesk.bzPTR
        Dim objWorkOrder As New AAMS.bizBOHelpDesk.bzWorkOrder
        Dim objRequestBDR As New AAMS.bizBOHelpDesk.bzRequestBDR
        Dim objIR As New AAMS.bizBOHelpDesk.bzIR
        Dim ds As New DataSet
        Dim objXmlReader As XmlNodeReader
...
byPassCondition:
    ...
    Sub BindDescription()
        ...
        Dim ds As New DataSet
        Dim objCall As New AAMS.bizBOHelpDesk.bzCall
    ...
    Private Sub ViewCallLogInsertDetails()
        ...
        If Session("Security") IsNot Nothing Then
            ...
            If objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default") IsNot Nothing Then
                If objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("FUN_ASSIGNEDTO").Value <> "" Then
                    HdDefaultTeamAssignedTo.Value = objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("FUN_ASSIGNEDTO").Value
                    li = ddlTeamAssignedTo.Items.FindByValue(objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("FUN_ASSIGNEDTO").Value)
                    If li IsNot Nothing Then
                        ddlTeamAssignedTo.SelectedValue = li.Value
                    End If
                End If
                If objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("CONTACT_TYPE_ID").Value <> "" Then
                    HdDefaultContactType.Value = objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("CONTACT_TYPE_ID").Value
                    li = ddlContactType.Items.FindByValue(objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("CONTACT_TYPE_ID").Value)
                    If li IsNot Nothing Then
    ...
    Sub AddCoordinator()
        Dim objOutputXml As New XmlDocument
        Dim objXmlDoc As New XmlDocument
        Dim objCordinator As New AAMS.bizBOHelpDesk.bzCoordinator
        Try
    ...
    Sub DownloadFile(ByVal strFileName As String)
        ...
        Try
            ...
            If strFileName.Trim = "" Then
                Exit Sub
            End If
            Dim fullpath As String = Path.GetFullPath(Server.MapPath("../BackOfficeHelpDeskFiles/") + strFileName)
    ...
    Public Sub GetCallerName(ByVal strCallerName As String)
