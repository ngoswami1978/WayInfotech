'########################################################################
'############   Page Name -- BOHDRPT_HelpDeskDynamicReport.aspx     #######
'############   Date 25-April 2011  #####################################
'############   Developed By Abhishek  ##################################
'########################################################################
Partial Class BOHelpDesk_HDRPT_HelpDeskDynamicReport
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If (Request.QueryString("Reload") IsNot Nothing) Then
                Response.Redirect("BOHDRPT_HelpDeskDynamicReport.aspx", False)
                Exit Sub
            End If
            Dim objSecurityXml As New XmlDocument
            objSecurityXml.LoadXml(Session("Security"))
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Dynamic Report']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Dynamic Report']").Attributes("Value").Value)
            ...
            If (Not IsPostBack) Then
                ...
                objeAAMS.BindDropDown(drpFollowup, "BOWorkOrderFollowUp", True, 3)
                ...
                If Session("LoginSession") IsNot Nothing Then
                    ...
                    Dim objbzDMReportConfiguration As New AAMS.bizBOHelpdesk.bzDMReportConfiguration
    ...
    Sub SearchRecords(ByVal Operation As Integer)
        ...
        If Session("LoginSession") IsNot Nothing Then
            ...
            If Operation = PageOperation.Search Then
                ...
                If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                    Session("BOHelpDeskDynamicReport") = objOutputXml.OuterXml
                    Response.Redirect("../RPSR_ReportShow.aspx?Case=BOHelpDeskDynamicReport", False)
                Else
                    lblError.Text = objOutputXml.DocumentElement.SelectSingleNode("Errors/Error").Attributes("Description").Value
                End If
            Else
                objOutputXml = objbzDMReportConfiguration.Export(objInputXml)
                Dim objExport As New ExportExcel
                If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                    Dim strHeader As String = objOutputXml.DocumentElement.SelectSingleNode("Header").Attributes("HAttributes").Value
                    Dim Headerlist As New ArrayList
                    Headerlist.AddRange(strHeader.Split(","c))

                    Dim c As Integer = Headerlist.Count - 1
                    If Headerlist.Contains("HD_RE_ID_D") Then c -= 1
                    If Headerlist.Contains("HD_RE_ID_S") Then c -= 1

                    Dim strArray(c) As String
                    Dim intArray(c) As Integer
                    Dim strAttribute As String

                    c = 0 '' Position of the Attibute
                    Dim j As Integer = 0 ' Position of the Array
                    For Each strAttribute In Headerlist
                        If strAttribute = "HD_RE_ID_D" Or strAttribute = "HD_RE_ID_S" Then
                        Else
                            strArray(j) = strAttribute
                            intArray(j) = c
                            j += 1
                        End If
                        c += 1
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Try
            '<HD_DMSEARCH_CALLREQUEST_OUTPUT>
            '      <SEARCHDETAILS LCODE="32755" LTR_NO="1" AGENCY="BALMER LAWRIE" ADDRESS="core 8,4th floor,scope building" PTR_NO="" QUERY_STATUS="Closed" FOLLOWUP="" SEVERITY="Severity1" LOGGED_BY="Anand Mohan Singh" OPEN_DATE_FROM="20-May-11 10:51" OPEN_DATE_TO="20-May-11 10:51" ASSIGNED_TO="DEL" OFFICE_ID="DELBL34E8" CUSTOMER_CATEGORY="" CALLER_NAME="Amit Kumar" QUERY_GROUP="Functional" QUERY_SUB_GROUP="Backoffice Main" QUERY_CATEGORY="Transaction" QUERY_SUB_CATEGORY="Invoice Generation" COORDINATOR1="Anand Mohan Singh" DISPOSITION="" AOFFICE="DEL" AGENCY_AOFFICE="DEL" WORK_ORDER_NO="" LINKLTR="" ATYPE="G" ASTATUS="TMC Travel Mgt Co" TITLE="" HD_IR_REF="" Desc1="invoice is not saving with changed fare." Desc2="" HD_RE_ID_D="1" Solu1="Open the internal link of B&amp;L and resave the same.... value got updated." Solu2="Open the internal link of B&amp;L and resave the same.... value got updated." Solu3="" HD_RE_ID_S="1" />
            '           <Header   HAttributes=''/>
            '               <Errors Status=''>	
