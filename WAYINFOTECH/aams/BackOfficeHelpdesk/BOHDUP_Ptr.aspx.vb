...
Partial Class BOHelpDesk_HDUP_Ptr
    ...
    Private Sub BindDropDowns()
        Try
            ...
            objeAAMS.BindDropDown(drpFollowup, "BOHDFOLLOWUP", True)
            objeAAMS.BindDropDown(drpType, "BOHDPTRTYPE", True)
            objeAAMS.BindDropDown(drpSeverity, "BOHDPTRSEVERITY", True)
            'objeAAMS.BindDropDown(drpStatus, "HDPTRCALLSTATUS_NOTUSE", False)
            objeAAMS.BindDropDown(drpStatus, "BOHDPTRSTATUSFORMANAGE", False)
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            If (Not Request.QueryString("Action") = Nothing) Then
                ...
                Dim objhdPtrSave As New AAMS.bizBOHelpDesk.bzPTR
                ...
                If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                    If (Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "U" Or Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "US") Then
                        ...
                        Else
                            If Request.QueryString("Popup") Is Nothing Then
                                ...
                                Response.Redirect("BOHDUP_Ptr.aspx?Action=US|" & objOutputXml.DocumentElement.SelectSingleNode("PTR").Attributes("HD_PTR_ID").Value + "&Msg=U", False)
                            Else

                                Response.Redirect("BOHDUP_Ptr.aspx?Action=US|" & objOutputXml.DocumentElement.SelectSingleNode("PTR").Attributes("HD_PTR_ID").Value + "&Popup=T", False)
                            End If
                    ...
                    Else
                        ...
                        End If
                        If Request.QueryString("Popup") Is Nothing Then
                            Response.Redirect("BOHDUP_Ptr.aspx?Action=US|" & hdEnPtrID.Value + "&LCode=" + hdEnLcode.Value, False)
                        Else
                            Response.Redirect("BOHDUP_Ptr.aspx?Action=US|" & hdEnPtrID.Value + "&Popup=T&ReqID=" + hdEnRequestID.Value + "&LCode=" + hdEnLcode.Value, False)
                        End If
                    End If
                Else
                    lblError.Text = objOutputXml.DocumentElement.SelectSingleNode("Errors/Error").Attributes("Description").Value
    ...
    Private Sub ViewPtrInsertDetails()
        ...
        If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
            ...
            For Each xNode In objOutputXml.DocumentElement.SelectNodes("CONFIGVALUE")
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_PTR_TYPE" Then
                    drpType.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim()
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_PTR_FOLLOWUP" Then
                    drpFollowup.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim()
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_PTR_SEVERITY" Then
                    drpSeverity.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim()
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_PTR_STATUS" Then
                    ' drpStatus.ClearSelection()
        ...
        Try
            If Request.QueryString("Action") IsNot Nothing Then
                If Request.QueryString("Action") = "I" Then
                    Response.Redirect("BOHDUP_Ptr.aspx?" + Request.QueryString.ToString)
                Else
                    Response.Redirect("BOHDUP_Ptr.aspx?Action=U|" + hdEnPtrID.Value.Trim())
                End If
            Else
                Response.Redirect("BOHDUP_Ptr.aspx?Action=U|" + hdEnPtrID.Value.Trim())
            End If
    ...
    Public Function ReturnPTR_statusValu() As String
        ...
        Dim objbzCallStatus As New AAMS.bizBOHelpDesk.bzPTR
    ...
    Protected Sub btnNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNew.Click
        If Request.QueryString("Popup") IsNot Nothing Then
            Response.Redirect("BOHDUP_PTR.aspx?Action=I&Popup=T&ReqID=" + objED.Encrypt(hdRequestID.Value) + "&LCode=" + objED.Encrypt(hdLcode.Value))
        Else
            Response.Redirect("BOHDUP_PTR.aspx?Action=I&ReqID=" + objED.Encrypt(hdRequestID.Value) + "&LCode=" + objED.Encrypt(hdLcode.Value))
        End If
...
#Region "Code for Paging "
    ...
    Private Sub BindListForPTR(ByVal strPTR As String)
        Dim objInputXml, objOutputXml As New XmlDocument
        Dim objbzRequestPTR As New AAMS.bizBOHelpDesk.bzPTR
