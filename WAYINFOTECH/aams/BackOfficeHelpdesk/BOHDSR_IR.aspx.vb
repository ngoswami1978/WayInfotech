...
Imports System.Xml
Partial Class BOHelpDesk_HDSR_IR
    ...
    Private Sub BindDropDowns()
        Try
            ...
            objEaams.BindDropDown(drpFollowup, "BOHDIRFOLLOWUP", True, 3)
            objEaams.BindDropDown(drpIRStatus, "BOHDIRSTATUS", True, 3)
            objEaams.BindDropDown(drpIRTypCat, "BOHDIRTYPECAT", True, 3)
            objEaams.BindDropDown(drpIRType, "BOHDIRTYPE", True, 3)
            objEaams.BindDropDown(drpSeverity, "BOHDIRSEVERITY", True, 3)
            objEaams.BindDropDown(drpAssignedTo, "BOHDIRASSIGNEDTO", True, 3)
        Catch ex As Exception
            lblError.Text = ex.Message
        End Try
    End Sub
    ...
    Private Sub GetSearchResult()
        ...
        Try
            ...
            End With
            Dim objHdIRSearch As New AAMS.bizBOHelpDesk.bzIR
    ...
    Protected Sub grdvIR_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdvIR.RowDataBound
        Try
            ...
            objSecurityXml.LoadXml(Session("Security"))
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='IR']").Count <> 0 Then
                    strBuilder = objeaams.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='IR']").Attributes("Value").Value)
                End If
    ...
    Private Sub HDIRDelelete(ByVal IRID As String)
        ...
        Try
            Dim objhdIRDel As New AAMS.bizBOHelpDesk.bzIR
