'#####################################################################
'######   Page Name -- HelpDesk_HDUP_RequestTypeCategory     #########  
...
Imports System.Data
#End Region
Partial Class BOHelpDesk_HDUP_RequestTypeCategory
...
#Region " Load City Data For Editing Mode"
    Sub ViewRequestTypeCategory()
        Try
            If (Not Request.QueryString("Action") = Nothing) Then
                Dim objInputXml, objOutputXml As New XmlDocument
                'Dim objXmlReader As XmlNodeReader
                Dim ds As New DataSet
                Dim objbzRequestTypeCategory As New AAMS.bizBOHelpDesk.bzRequestTypeCategory
                ...
                If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                    BindRequestTypeCategoryData(objOutputXml)
                End If
            End If
        Catch ex As Exception
            lblError.Text = ex.Message
    ...
    End Sub
#End Region
#Region "Call BindRequestTypeCategoryData For Binding Data in Controls"
    Sub BindRequestTypeCategoryData(ByVal objOutputXml As XmlDocument)
        Try
            txtCategory.Text = objOutputXml.DocumentElement.SelectSingleNode("RequestCategory").Attributes("HD_RE_TYCAT_NAME").InnerXml
                ...
                If (Not Request.QueryString("Action") = Nothing) Then
                    ...
                    If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                        If (Request.QueryString("Action") = "U") Then
                            ...
                            Response.Redirect("BOHDUP_RequestTypeCategory.aspx?&Msg=U&Action=U&HD_RE_TYCAT_ID=" & objOutputXml.DocumentElement.SelectSingleNode("RequestCategory").Attributes("HD_RE_TYCAT_ID").Value, False)
                        Else
                            lblError.Text = objeAAMSMessage.messInsert '"Record added successfully."
                            Response.Redirect("BOHDUP_RequestTypeCategory.aspx?&Msg=A&Action=U&HD_RE_TYCAT_ID=" & objOutputXml.DocumentElement.SelectSingleNode("RequestCategory").Attributes("HD_RE_TYCAT_ID").Value, False)
                        End If
                    Else
