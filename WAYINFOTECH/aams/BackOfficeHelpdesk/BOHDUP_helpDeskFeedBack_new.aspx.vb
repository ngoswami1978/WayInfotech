...
Partial Class BOHelpDesk_HDUP_helpDeskFeedBack_new
    ...
    Sub checkSecurity()
        ...
        objSecurityXml.LoadXml(Session("Security"))
        If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
            If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Feedback Action']").Count <> 0 Then
                strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO Feedback Action']").Attributes("Value").Value)
            End If
    ...
    Private Sub ViewFeedbackDetails()
        ...
        objSecurityXml.LoadXml(Session("Security"))
        If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
            If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='Feedback Action Supervisory']").Count <> 0 Then
                strBuildersupervisory = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='Feedback Action Supervisory']").Attributes("Value").Value)
            Else
        ...
        Dim objhdFeedbackView As New AAMS.bizBOHelpDesk.bzFeedback
