...
Partial Class BOHelpDesk_TCUP_CallLog
    ...
    Sub LoadAllControl()
        Try
            objeAAMS.BindDropDown(ddlQuerySubGroup, "BOTQUERYSUBGROUP", True, 1)
            objeAAMS.BindDropDown(ddlQueryStatus, "BOTSTATUSWITHCLOSED", True, 1)
            objeAAMS.BindDropDown(ddlPriority, "BOTPRIORITY", True, 1)
            objeAAMS.BindDropDown(ddlTeamAssignedTo, "BOASSIGNEDTOEMPLOYEE", True, 1)
    ...
    Protected Sub theTabStrip_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles theTabStrip.ItemDataBound
        ...
        If Session("Security") IsNot Nothing Then
            ...
            Select Case strButtonText
                Case "Call"
                    If objeAAMS.ReturnViewPermission(Session("Security"), "BO Log Tech Call") = "0" Then
    ...
    Protected Sub btnNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        Response.Redirect("BOTCUP_CallLog.aspx?Action=I")
    ...
    Protected Sub rdFunctional_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rdFunctional.CheckedChanged
        ...
        If hdPageStatus.Value = "I" Or hdPageStatus.Value = "" Then
            ...
            If hdCallAgencyName.Value <> "" Then
                ...
                'End If
                Response.Redirect("BOHDUP_Calllog.aspx?LCodeTemp=" + hdEnCallAgencyName_LCODE.Value)
            Else
                Response.Redirect("BOHDUP_Calllog.aspx?Action=I") '+ hdQueryString.Value)
            End If
        Else
            Response.Redirect("BOHDUP_Calllog.aspx?" + hdQueryString.Value)
        End If
        'New End Code
    ...
    Private Sub ViewCallLogInsertDetails()
        ...
        If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
            ...
            For Each xNode In objOutputXml.DocumentElement.SelectNodes("CONFIGVALUE")
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_REQUEST_SEVERITY" Then
                    ddlPriority.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim().Split("|").GetValue(1)
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_REQUEST_STATUS" Then
                    ...
                    End If
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_REQUEST_CONTACT_TYPE" Then
        ...
        If Session("Security") IsNot Nothing Then
            ...
            If objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default") IsNot Nothing Then
                If objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("TEC_ASSIGNEDTO").Value <> "" Then
                    HdTechDefaultTeamAssignedTo.Value = objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("TEC_ASSIGNEDTO").Value
                    li = ddlTeamAssignedTo.Items.FindByValue(objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("TEC_ASSIGNEDTO").Value)
                    If li IsNot Nothing Then
                        ddlTeamAssignedTo.SelectedValue = li.Value
                    End If
                End If
                If objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("TEC_CONTACT_TYPE_ID").Value <> "" Then
                    HdTechDefaultContactType.Value = objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("TEC_CONTACT_TYPE_ID").Value
                    li = ddlContactType.Items.FindByValue(objOutputXml.DocumentElement.SelectSingleNode("BOHelpDesk_Default/HD_Default").Attributes("TEC_CONTACT_TYPE_ID").Value)
                    If li IsNot Nothing Then
