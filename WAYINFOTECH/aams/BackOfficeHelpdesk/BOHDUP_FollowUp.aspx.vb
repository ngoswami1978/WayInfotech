...
Partial Class BOHelpDesk_HDUP_FollowUp
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not Page.IsPostBack Then
                ...
                objeAAMS.BindDropDown(ddlMode, "CONTACTTYPE", True)
                BindRep()
                BindData()
            End If
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim objCall As New AAMS.bizBOHelpDesk.bzCall
            ' Load Input Xml.
            ' objeAAMS.ConvertTextDate(txtAssignedDate.Text)
            objInputXml.LoadXml("<HD_UPDATEREQUESTFOLLOWUP_INPUT><REQUESTFOLLOWUP HD_RE_ID ='' EmployeeID='' CONTACT_TYPE_ID='' FOLLOWUP_DESC='' DATETIME='' /></HD_UPDATEREQUESTFOLLOWUP_INPUT>")
            objInputXml.DocumentElement.SelectSingleNode("REQUESTFOLLOWUP").Attributes("HD_RE_ID").Value = hdPageHD_RE_ID.Value
            objInputXml.DocumentElement.SelectSingleNode("REQUESTFOLLOWUP").Attributes("EmployeeID").Value = Session("LoginSession").ToString().Split("|")(0)
            objInputXml.DocumentElement.SelectSingleNode("REQUESTFOLLOWUP").Attributes("FOLLOWUP_DESC").Value = txtDescription.Text
            objInputXml.DocumentElement.SelectSingleNode("REQUESTFOLLOWUP").Attributes("CONTACT_TYPE_ID").Value = ddlMode.SelectedValue
            objInputXml.DocumentElement.SelectSingleNode("REQUESTFOLLOWUP").Attributes("DATETIME").Value = objeAAMS.GetDateFormat(txtNxtFollowupDate.Text, "dd/MM/yyyy HH:mm", "MM/dd/yyyy HH:mm", "/")
            ' Calling update method for update.
            objOutputXml = objCall.SaveRequestFollowup(objInputXml)
