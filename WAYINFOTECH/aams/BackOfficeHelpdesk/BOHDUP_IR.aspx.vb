...
Imports System.Xml
Partial Class BOHelpDesk_HDUP_IR
    ...
    Private Sub ViewIRDetails()
        Try
            ...
            Dim objhdIRView As New AAMS.bizBOHelpDesk.bzIR
    ...
    Private Sub BindDropDowns()
        Try
            ...
            objeAAMS.BindDropDown(drpFollowup, "BOHDIRFOLLOWUP", True)
            objeAAMS.BindDropDown(drpType, "BOHDIRTYPE", True)
            objeAAMS.BindDropDown(drpSeverity, "BOHDIRSEVERITY", True)
            objeAAMS.BindDropDown(drpStatus, "BOHDIRSTATUSFORMANAGE", False)
            objeAAMS.BindDropDown(drpAssignedTo, "BOHDASSIGNEDTO", True)
        Catch ex As Exception
            lblError.Text = ex.Message
        End Try
    End Sub
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            If (Not Request.QueryString("Action") = Nothing) Then
                ...
                Dim objhdIRSave As New AAMS.bizBOHelpDesk.bzIR
                ...
                If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
                    If (Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "U" Or Request.QueryString("Action").ToString().Split("|").GetValue(0).ToString().ToUpper() = "US") Then
                        ...
                        Else
                            If Request.QueryString("Popup") Is Nothing Then
                                ...
                                Response.Redirect("BOHDUP_IR.aspx?Action=US|" & objOutputXml.DocumentElement.SelectSingleNode("IR").Attributes("HD_IR_ID").Value + "&Msg=U", False)
                            Else

                                Response.Redirect("BOHDUP_IR.aspx?Action=US|" & objOutputXml.DocumentElement.SelectSingleNode("IR").Attributes("HD_IR_ID").Value + "&Popup=T", False)
                            End If
                    ...
                    Else
                        ...
                        End If
                        If Request.QueryString("Popup") Is Nothing Then
                            Response.Redirect("BOHDUP_IR.aspx?Action=US|" & hdEnIRID.Value + "&LCode=" + hdEnLcode.Value, False)
                        Else
                            Response.Redirect("BOHDUP_IR.aspx?Action=US|" & hdEnIRID.Value + "&Popup=T&ReqID=" + hdEnRequestID.Value + "&LCode=" + hdEnLcode.Value, False)
                        End If
                    End If
                Else
                    lblError.Text = objOutputXml.DocumentElement.SelectSingleNode("Errors/Error").Attributes("Description").Value
    ...
    Private Sub ViewIRInsertDetails()
        ...
        If objOutputXml.DocumentElement.SelectSingleNode("Errors").Attributes("Status").Value.ToUpper = "FALSE" Then
            ...
            For Each xNode In objOutputXml.DocumentElement.SelectNodes("CONFIGVALUE")
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_IR_TYPE" Then
                    drpType.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim()
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_IR_FOLLOWUP" Then
                    drpFollowup.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim()
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_IR_SEVERITY" Then
                    drpSeverity.SelectedValue = xNode.Attributes("FIELD_VALUE").Value.Trim()
                End If
                If xNode.Attributes("FIELD_NAME").Value.Trim() = "DEFAULT_BOHD_IR_STATUS" Then
                    ' drpStatus.ClearSelection()
        ...
        Try
            If Request.QueryString("Action") IsNot Nothing Then
                If Request.QueryString("Action") = "I" Then
                    Response.Redirect("BOHDUP_IR.aspx?" + Request.QueryString.ToString)
                    'If Request.QueryString("Popup") IsNot Nothing Then
                    ...
                    'End If
                Else
                    Response.Redirect("BOHDUP_IR.aspx?Action=U|" + hdEnIRID.Value.Trim())
                End If
            Else
    ...
    Private Sub CheckSecurity()
        Try
            ...
            If (objSecurityXml.DocumentElement.SelectSingleNode("Administrator").InnerText = "0") Then
                If objSecurityXml.DocumentElement.SelectNodes("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO IR']").Count <> 0 Then
                    strBuilder = objeAAMS.SecurityCheck(objSecurityXml.DocumentElement.SelectSingleNode("SECURITY_OPTIONS/SECURITY_OPTION[@SecurityOptionSubName='BO IR']").Attributes("Value").Value)
                End If
    ...
    Public Function ReturnIR_statusValu() As String
        ...
        Dim objbzCallStatus As New AAMS.bizBOHelpDesk.bzIR
    ...
    Private Function CallStatus() As ListItem
        Dim objHdStatis As New AAMS.bizBOHelpDesk.bzCallStatus
    ...
    Protected Sub btnNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNew.Click
        If Request.QueryString("Popup") IsNot Nothing Then
            Response.Redirect("BOHDUP_IR.aspx?Action=I&Popup=T&ReqID=" + objED.Encrypt(hdRequestID.Value) + "&LCode=" + objED.Encrypt(hdLcode.Value))
        Else
            Response.Redirect("BOHDUP_IR.aspx?Action=I&ReqID=" + objED.Encrypt(hdRequestID.Value) + "&LCode=" + objED.Encrypt(hdLcode.Value))
        End If
...
#Region "Code for Paging "
    ...
    Private Sub BindListForIR(ByVal strIR As String)
        Dim objInputXml, objOutputXml As New XmlDocument
        Dim objbzRequestIR As New AAMS.bizBOHelpDesk.bzIR
